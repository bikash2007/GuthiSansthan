  ...
  return (
    <>
      ...
      >
        ...
        >
          {isEditing && (
            <>
              <div
                className="absolute top-0 left-0 bg-red-600 cursor-pointer rounded-b-md px-2 py-1 text-white hover:bg-red-700 z-30"
                onClick={removeParva}
              >
                Remove
              </div>
              <div
                className="absolute top-0 right-0 bg-blue-600 cursor-pointer rounded-md px-2 py-1 text-white hover:bg-blue-700 z-30"
                onClick={acitvateEditing}
              >
                Edit
              </div>
            </>
      ...
      </div>
      {
        <motion.div
          className={`${
            isHidden ? "h-0 w-0" : "h-[60vh] w-[90%]"
          } absolute top-20 rounded-xl bg-neutral-800 flex flex-col items-center justify-start z-50 backdrop-blur-3xl overflow-auto transition-all duration-200 ease-out`}
        >
          <FontAwesomeIcon
            icon={faClose}
            size={"2x"}
          ...
          {isParvaEditingActivate && (
            <EditParva
              ...
              detail={detail}
              img={img}
              fetchAllParva={fetchAllParva}
              parvaPageDetail={parvaPageDetail}
            />
          )}
        </motion.div>
      }
