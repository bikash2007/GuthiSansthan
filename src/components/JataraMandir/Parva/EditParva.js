...
export const EditParva = ({
  ...
  const handleImage = (event) => {
    ...
    if (selectedFile) {
      setParva(URL.createObjectURL(selectedFile));
  ...
  const handleQrCode = (event) => {
    const selectedFile = event.target.files[0];
    if (selectedFile) {
      setQrCodeFile(URL.createObjectURL(selectedFile));
      setQrCodeFile(selectedFile);
    }
  };
        ...
        <div className="w-full flex flex-col items-center my-4">
          ...
          <div className="relative">
            <label
              ...
              className="border-b bg-gray-500 border-black rounded-md w-56 min-h-56 flex justify-center items-center cursor-pointer"
              style={{
                backgroundImage: `url(${qrCodeFile || qrCode})`,
                backgroundSize: "cover",
                backgroundPosition: "center",
              }}
            >
