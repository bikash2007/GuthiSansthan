...
import {
  setImageUrl,
  setQRUrl,
} from "../../../state/HomePageSlices/TempleSlice";
...
const InstanceTemple = ({
  ...
  const removeTemple = () => {
    try {
      const deleteData = async () => {
        try {
          const confirmDelete = await showConfirmBox(
  ...
  return (
    <>
      ...
      >
        <div
          className="relative h-full w-full flex items-center justify-center bg-cover bg-center overflow-hidden rounded-md"
          style={{
            backgroundImage: `url(${templeDetail.dynamicDetails[index].image})`,
          }}
        >
          {isEditing && (
            <>
              <div
                className="absolute top-0 left-0 bg-red-600 cursor-pointer rounded-md px-2 py-1 text-white hover:bg-red-700 z-30"
                onClick={removeTemple}
              >
                Remove
              </div>
              <div
                className="absolute top-0 right-0 bg-blue-600 cursor-pointer rounded-md px-4 py-1 text-white hover:bg-blue-700 z-30"
      ...
      </div>
      {
        <motion.div
          className={`${
            isHidden ? "h-0 w-0" : "h-[87%] w-[98%] md:w-[95%] lg:w-[90%]"
          } absolute top-20 rounded-xl bg-neutral-900/100 flex flex-col items-center justify-start z-40 backdrop-blur-lg overflow-auto transition-all duration-200 ease-out`}
        >
          <FontAwesomeIcon
            icon={faClose}
            size={"2x"}
