...
export const HomePageSlice = createSlice({
  ...
  initialState: {
    ...
    "rules-and-regulation-tab": {
      isFetched: false,
      imgSrc: "",
      id: "",
      actualImgSrc: "",
    },
    "welcome-to-guthi-sansthan": {
      id: "",
  ...
  reducers: {
    ...
    setTabDetail: (state, action) => {
      state[action.payload.name] = getImageDetail(
        action.payload.detail,
        state["details"][action.payload.name].id
      );
    },
    setNewTabDetail: (state, action) => {
      if (action.payload.detail)
        state[action.payload.name].imgSrc = action.payload.detail;
      else
        state[action.payload.name].imgSrc =
          state[action.payload.name].actualImgSrc;
    },
